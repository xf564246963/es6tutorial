...
==lookahead的另一种翻译：环视。一共分为4种：肯定顺序环视（positive-lookahead）、否定顺序环视（negative-lookahead）、肯定逆序环视（positive-lookbehind）、否定逆序环视（negative-lookbehind）。==

==先行断言和后行断言助记：断言匹配的是字符之间的空隙（见下图）,先行断言是正则匹配到当前空隙位置按从左往右的顺序判断，后行断言是从当前空隙位置向左回溯判断==

![lookahead](./../images/epub_25462636_182.jpg)

==环视（或断言）的最大特点是“匹配完成之后还停在原地”==
...
&nbsp;&nbsp;&nbsp;&nbsp;==（这里后行断言是先判断$之后，从字符结束处从右往左匹配，所以先判断第二个括号的(\d+)，后判断第一个括号的(\d+)）==

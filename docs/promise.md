...
（1）对象的状态不受外界影响。`Promise`对象代表一个异步操作，有==三种状态：`pending`（进行中）、`fulfilled`（已成功）和`rejected`（已失败）==。只有异步操作的结果，可以决定当前是哪一种状态，任何其他操作都无法改变这个状态。这也是`Promise`这个名字的由来，它的英语意思就是“承诺”，表示其他手段无法改变。
...
==<说明采用采用链式的then，尽管then方法返回的是一个新的Promise实例，但是then的return如果是同步结果，会同步依次触发，这跟直接new Promise(...).then是在宏任务执行完成之后才触发回调不一样>==

```javascript
console.log("start script");
setInterval(() => { // 用定时器实现宏任务不断插入
...
上面代码中，==`promise`抛出一个错误，就被`catch()`方法指定的回调函数捕获（catch只能处理promise内的错误--Promise实例传参和then回调）==。注意，上面的写法与下面两种写法是等价的。

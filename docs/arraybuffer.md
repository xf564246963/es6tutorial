...
==`buffer`的字节长度`byteLength`是16个字节，`Int32Array`的`BYTES_PER_ELEMENT`是4字节，`int32View`的`length`是4。`Int16Array`的`BYTES_PER_ELEMENT`是2字节，`int16View`的`length`是8。以数字2为例，在`Int32Array`的存储为`00000000 00000000 0000000 00000002`，以`Int16Array`数组表示需要分为两个16位的二进制`00000000 00000000`和`0000000 00000002`，由于小端字节序的原因，`0000000 00000002`在`00000000 00000000`之前。==
...
// 从Uint8Array转Uint16Array，根据小端字节序定义，低位在前
// 得到[0x02, 0x01, 0x03, 0x07] => [0x0102, 0x0703] 对应 258 1795
...
function getPlatformEndianness() {
  ...
  let arr8 = new Uint8Array(arr32.buffer); //小端字节序 [0x78, 0x56, 0x34,0x12]

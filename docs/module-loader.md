...
==**这里和commonjs一样，当文件a.mjs已经加载过了，就不会再继续加载a.mjs，而只是加载了部分代码，当继续加载b.mjs，等b.mjs加载完之后再继续加载剩下的a.mjs==
...
==<变量提升”意味着变量和函数的声明会在物理层面移动到代码的最前面，正如我们所模拟的那样。但，这并不准确。实际上变量和函数声明在代码里的位置是不会改变的，而且是在编译阶段被 JavaScript 引擎放入内存中。对，你没听错，一段 JavaScript 代码在执行之前需要被 JavaScript 引擎编译，编译完成之后，才会进入执行阶段。>==

![variable-hoist](./../images/variable-hoist.png)

==**import和export会在静态分析阶段生成一个名字记录表，形成一个可读的引用；变量和函数声明也会被提前被解析引擎放入到内存中执行。从代码执行的角度来看，变量提升至import之前，或者说变量提升之后import的引用被更新了，import的变量如果被提升了就总是会被直接获取到。**（通过上面循环引用的例子推理得到）==

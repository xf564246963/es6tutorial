...
==函数也一样，函数Me这个名字只能在函数内部被使用，在函数外部只能MyFunction这个引用==
```javascript
let MyFunction = function Me() {

}
console.log(MyFunction.name); // Me
// 只有函数是匿名(Anonymous)的且是定义表达式(IsFunctionDefinition),
// 才会重新绑定function.name
console.log(Me.name); // Uncaught ReferenceError: Me is not defined
// 具名函数作为函数表达式时，名字在块级作用域中无意义
```
